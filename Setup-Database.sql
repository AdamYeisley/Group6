...
-- Currency
  ...
  IF error_msg <> '' THEN
    ...
    VALUES (
      'employee',
      NULL,                       -- or a specific field name if you prefer
      NULL,                       -- optional: a single raw value
      to_jsonb(NEW),              -- full row as JSONB
      trim(both ' ' from v_error_msg)
    );

    RAISE EXCEPTION 'Invalid employee insert: %', error_msg;
  END IF;
